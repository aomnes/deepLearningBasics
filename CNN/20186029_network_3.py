...
def custom_model_fn(features, labels, mode):
    ...
    dropout1 = tf.layers.dropout(inputs = max_pooling_1, rate=dropout_rate, training=mode == tf.estimator.ModeKeys.TRAIN)
    ...
    dropout2 = tf.layers.dropout(inputs = max_pooling_2, rate=dropout_rate, training=mode == tf.estimator.ModeKeys.TRAIN)


    resize = tf.reshape(dropout2, [-1, 7 * 7 * 64])
    hidden3 = tf.layers.dense(resize, 1024, name="hidden3", activation=tf.nn.relu)
    dropout3 = tf.layers.dropout(inputs = hidden3, rate=dropout_rate, training=mode == tf.estimator.ModeKeys.TRAIN)

    # Output logits Layer
    logits = tf.layers.dense(inputs=dropout3, units=output_layer_size, name="output_logits", activation=None) #Size [batch_size, 10]
...
if __name__ == '__main__':
    ...
    np.save('20186029_network_5.npy', result)
