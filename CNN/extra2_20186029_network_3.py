...
def custom_model_fn(features, labels, mode):
    ...
    max_pooling_1 = tf.layers.max_pooling2d(layer_conv1, pool_size=[2, 2], strides=2)


    layer_conv2 = tf.layers.conv2d(max_pooling_1, filters=96, kernel_size=[3, 3], padding='same', activation=tf.nn.relu)
    max_pooling_2 = tf.layers.max_pooling2d(layer_conv2, pool_size=[2, 2], strides=2)
    dropout1 = tf.layers.dropout(inputs = max_pooling_2, rate=0.2, training=mode == tf.estimator.ModeKeys.TRAIN)

    resize = tf.reshape(dropout1, [-1, 8 * 8 * 96])
    #norm = tf.nn.batch_normalization(resize, training=mode == tf.estimator.ModeKeys.TRAIN)
    dense6 = tf.layers.dense(resize, 2048 , name="dense1", activation=tf.nn.relu)
...
if __name__ == '__main__':
    ...
    np.save('extra2_20186029_network_3.npy', result)
