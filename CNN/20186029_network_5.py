...
def custom_model_fn(features, labels, mode):
    ...
    resize = tf.reshape(dropout4, [-1, 7 * 7 * 128])
    hidden5 = tf.layers.dense(resize, 1024, name="hidden3", activation=tf.nn.relu)
    dropout5 = tf.layers.dropout(inputs = hidden5, rate=0.4, training=mode == tf.estimator.ModeKeys.TRAIN)

    # Output logits Layer
    logits = tf.layers.dense(inputs=dropout5, units=output_layer_size, name="output_logits", activation=None) #Size [batch_size, 10]
